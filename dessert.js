const api =
  'https://api.edamam.com/search?q=dessert&app_id=5d4c78dd&app_key=a22eff998c59bb561936f410722f1fa8&to=100';
...
fetch(api)
  ...
  .then((data) => {
    ...
    for (let i = 0; i < data.hits.length; i++) {
      const dessertpick = document.getElementById('dessertpick');
      const dessertbox = document.createElement('div');
      dessertbox.classList.add('dessertbox');
      ...
      dessertbox.appendChild(recipetitle);
      dessertbox.appendChild(image);
      dessertbox.appendChild(button);
      dessertpick.appendChild(dessertbox);
      dessertpick.appendChild(modal);
    }
  });
